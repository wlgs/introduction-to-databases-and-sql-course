...
SELECT T.TableID,
        T.Size
FROM TABLES T
        INNER JOIN ReservationDetails RD ON RD.TableID = T.TableID
...
SELECT O.OrderID,
        O.ClientID
FROM ORDERS O
        INNER JOIN OrdersTakeaways OT ON OT.TakeawayID = O.TakeawayID
WHERE O.Paid = 1
...
FROM DiscountVars DV
        INNER JOIN DiscountDetails DD ON DD.VarID = DV.VarID
GROUP BY DV.VarName,
        DD.Value,
        YEAR(DD.EndDate),
        MONTH(DD.EndDate);
...
GROUP BY P.Name,
        M.Price,
        YEAR(M.EndDate),
        DATEPART(week, M.EndDate) CREATE VIEW WeeklyTables AS
SELECT T.TableID,
        T.Size,
        YEAR(R.EndDate) year,
...
GROUP BY T.TableID,
        T.Size,
        YEAR(R.EndDate),
        DATEPART(week, R.EndDate) CREATE VIEW WeeklyDiscounts AS
SELECT DV.VarName,
        DD.Value,
...
GROUP BY P.Name
ORDER BY COUNT(OD.ProductID) DESC CREATE VIEW LeastPopularMealsAllTime AS
SELECT TOP 5 P.NAME
FROM Products P
        INNER JOIN OrderDetails OD on P.ProductID = OD.ProductID
        INNER JOIN Orders O on OD.OrderID = O.OrderID
GROUP BY P.Name
ORDER BY COUNT(OD.ProductID) CREATE VIEW MostPopularMealPerMenu AS
SELECT M.MenuID,
        M.StartDate,
        M.EndDate,
        (
...
FROM Menu M CREATE VIEW LeastPopularMealPerMenu AS
SELECT M.MenuID,
        M.StartDate,
        M.EndDate,
        (
...
FROM Menu M CREATE VIEW BestIndividualCustomersByAmountOrdered AS
...
GROUP BY O.ClientID,
        P.FirstName,
        P.LastName CREATE VIEW BestIndividualCustomersByValueOrdered AS
SELECT TOP 5 O.ClientID,
        P.FirstName,
        P.LastName,
...
FROM Orders O
        ...
        INNER JOIN Products P2 ON P2.ProductID = OD.ProductID
        INNER JOIN Menu M ON M.ProductID = P2.ProductID
GROUP BY O.ClientID,
        P.FirstName,
        P.LastName
ORDER BY SUM(OD.Quantity * M.Price) DESC CREATE VIEW BestCompaniesByAmountOrdered AS
...
ORDER BY Count(O.OrderID) DESC CREATE VIEW BestCompaniesByValueOrdered AS
SELECT TOP 5 CO.CompanyName,
        SUM(OD.Quantity * M.Price) total_sum
FROM Orders O
        INNER JOIN Clients C ON C.ClientID = O.ClientID
